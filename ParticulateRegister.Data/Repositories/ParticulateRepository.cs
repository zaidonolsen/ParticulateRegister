...
namespace ParticulateRegister.Data.Repositories
{
    public class ParticulateRepository : IParticulateRepository
    {
        ...
        public async Task<IEnumerable<ParticulateDto>> GetAllAsync()
            => await _context.Particulates.ToListAsync();
        ...
        //	EF Core InMemory does not automatically track new child entities in collections.
        public async Task AddHistoryAsync(ParticulateHistoryDto history)
        {
            await _context.ParticulateHistories.AddAsync(history);
            await _context.SaveChangesAsync();
        }
    }
}
