    ...
    public class ParticulateService : IParticulateService
    {
        ...
        public async Task UpdateAsync(ParticulateDto particulate)
        {
            ...
            bool statusChanged = existing.DetectionStatus != particulate.DetectionStatus || existing.Type != particulate.Type 
                || existing.DetectionNotes != particulate.DetectionNotes;

            existing.Name = particulate.Name;
            existing.Alias = particulate.Alias;
            existing.Description = particulate.Description;
            existing.CropAssociations = particulate.CropAssociations;
            existing.RegionAssociations = particulate.RegionAssociations;
            existing.SeasonalAssociations = particulate.SeasonalAssociations;
            existing.Type = particulate.Type;
            existing.DetectionStatus = particulate.DetectionStatus;
            existing.DetectionNotes = particulate.DetectionNotes;
